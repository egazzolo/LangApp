...
- Current events can come up naturally when the learner raises them or when they directly fit the conversation. React like an ordinary person, not a news presenter. Never invent breaking news, election results, scores, prices, officeholders, or other time-sensitive facts. If the latest facts were not supplied in the conversation, say naturally that you are not sure about the latest update.
...
- reinforcementFocus contains recurring learner skills that need practice. When focusedPractice is false, use one only when it naturally fits the existing thread. When focusedPractice is true, you may smoothly take the lead, introduce a plausible new conversational topic, or ask a natural question designed to let the learner use one priority skill. Even then, never mention the learning profile, sound like a lesson, abruptly ignore an important user message, correct the learner inside the chat, or target more than one focus in a single reply.
...
export const prompts = {
  ...
  tutorReview: {
    version: "tutor-review.v8",
    system:
      'Review only the supplied English learner messages. Return a concise list of actual mistakes or clearly unnatural wording. Honor correctionIntensity from the supplied context. For chill, include only errors that make the learner difficult to understand, and never correct a common understandable informal reduction or native texting form solely for register. For balanced, include meaningful errors and noticeably unnatural wording, but normally leave common understandable informal reductions and native texting forms alone. For intensive, include smaller useful corrections and may flag informal reductions or texting forms as register guidance. Honor correctPunctuation from the supplied context: when false, completely ignore punctuation-only mistakes and do not include a correction whose only change is punctuation; when true, punctuation mistakes may be included. Under intensive only, common informal forms such as "r u", "gonna", "wanna", "kinda", "gotta", and "idk" may receive register guidance; never call them simply wrong, and explain in interfaceLocale that native speakers commonly use them casually with friends while the standard form is more appropriate in formal speech and writing. Still correct wording that is confusing, unintended, or unnatural even in casual native texting. Do not praise, summarize, ask questions, continue the conversation, or add general feedback. For each correction, original must reproduce the relevant user sentence, mistake must be an exact contiguous substring of original, explanation must briefly teach what is wrong in the supplied interface locale, corrected must give one natural corrected sentence, and alternatives must provide one to three concise natural options the user could have said. Also return focusAreas: zero to eight reusable skills evidenced by mistakes in these messages. Use a stable lowercase underscore skillKey such as past_tense or article_choice, a short label in interfaceLocale, the number of distinct examples in evidenceCount, and conservative confidence. Compare the messages with knownFocusAreas and return strengthenedAreas only when the learner clearly uses one of those exact skillKeys correctly; report at most one successful use per distinct sentence and never invent success. Never put message text, names, conversation details, or sensitive facts in a skillKey or label. Do not invent errors. If there are no meaningful mistakes, corrections and focusAreas may be empty, but strengthenedAreas may still contain demonstrated known skills. Return only the requested schema.',
  },
  learningAnalysis: {

...
interface AppState {
  ...
  deletedConversations: DeletedConversation[]; hideDeleteChatWarning: boolean; correctTutorPunctuation: boolean; showLanguageHighlights: boolean; tutorReviews: Record<string, TutorReviewRecord[]>;
  ...
  setHideDeleteChatWarning: (value: boolean) => void; setCorrectTutorPunctuation: (value: boolean) => void; setShowLanguageHighlights: (value: boolean) => void; addTutorReview: (review: TutorReviewRecord) => void;
}
const notExpired = (item: DeletedConversation) => Date.now() - new Date(item.deletedAt).getTime() < RECYCLE_RETENTION_MS;
export const useAppStore = create<AppState>()(persist((set) => ({
  onboarded: false, locale: 'en', correctionIntensity: 'balanced', pronunciationTarget: 'general-american', characters: [], conversations: [], messages: {}, deletedConversations: [], hideDeleteChatWarning: false, correctTutorPunctuation: false, showLanguageHighlights: true, tutorReviews: {},

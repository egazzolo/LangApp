...
interface AppState {
  onboarded: boolean; locale: InterfaceLocale; hasSelectedInterfaceLanguage: boolean; correctionIntensity: CorrectionIntensity; pronunciationTarget: PronunciationTarget;
  characters: Character[]; conversations: Conversation[]; messages: Record<string, Message[]>;
  deletedConversations: DeletedConversation[]; hideDeleteChatWarning: boolean; correctTutorPunctuation: boolean; acceptCasualTexting: boolean; showLanguageHighlights: boolean; notificationsEnabled: boolean; voiceEnabled: boolean; tutorReviews: Record<string, TutorReviewRecord[]>;
  completeOnboarding: () => void; setLocale: (locale: InterfaceLocale) => void; confirmInterfaceLanguage: () => void; setCorrectionIntensity: (value: CorrectionIntensity) => void; setPronunciationTarget: (value: PronunciationTarget) => void;
  ...
  setHideDeleteChatWarning: (value: boolean) => void; setCorrectTutorPunctuation: (value: boolean) => void; setAcceptCasualTexting: (value: boolean) => void; setShowLanguageHighlights: (value: boolean) => void; setNotificationsEnabled: (value: boolean) => void; setVoiceEnabled: (value: boolean) => void; addTutorReview: (review: TutorReviewRecord) => void;
  resetForSignOut: () => void;
}
const notExpired = (item: DeletedConversation) => Date.now() - new Date(item.deletedAt).getTime() < RECYCLE_RETENTION_MS;
export const useAppStore = create<AppState>()(persist((set) => ({
  onboarded: false, locale: 'en', hasSelectedInterfaceLanguage: false, correctionIntensity: 'balanced', pronunciationTarget: 'general-american', characters: [], conversations: [], messages: {}, deletedConversations: [], hideDeleteChatWarning: false, correctTutorPunctuation: false, acceptCasualTexting: false, showLanguageHighlights: true, notificationsEnabled: true, voiceEnabled: true, tutorReviews: {},
  completeOnboarding: () => set({ onboarded: true }), setLocale: (locale) => set({ locale }), confirmInterfaceLanguage: () => set({ hasSelectedInterfaceLanguage: true }), setCorrectionIntensity: (correctionIntensity) => set({ correctionIntensity }), setPronunciationTarget: (pronunciationTarget) => set({ pronunciationTarget }),
  ...
  resetForSignOut: () => set({ onboarded: false, characters: [], conversations: [], messages: {}, deletedConversations: [], tutorReviews: {}, acceptCasualTexting: false }),
}), { name: 'langapp-state-v1', version: 9, storage: createJSONStorage(() => AsyncStorage), migrate: (persisted) => { const saved = persisted as Partial<AppState>; return { ...saved, hasSelectedInterfaceLanguage: saved.hasSelectedInterfaceLanguage ?? false, notificationsEnabled: saved.notificationsEnabled ?? true, voiceEnabled: saved.voiceEnabled ?? true, acceptCasualTexting: saved.acceptCasualTexting ?? false, characters: (saved.characters ?? []).map((character) => ({ ...character, gender: character.gender ?? legacyGender(character.name) })) }; }, partialize: (state) => ({ onboarded: state.onboarded, locale: state.locale, hasSelectedInterfaceLanguage: state.hasSelectedInterfaceLanguage, correctionIntensity: state.correctionIntensity, pronunciationTarget: state.pronunciationTarget, characters: state.characters, conversations: state.conversations, messages: state.messages, deletedConversations: state.deletedConversations, hideDeleteChatWarning: state.hideDeleteChatWarning, correctTutorPunctuation: state.correctTutorPunctuation, acceptCasualTexting: state.acceptCasualTexting, showLanguageHighlights: state.showLanguageHighlights, notificationsEnabled: state.notificationsEnabled, voiceEnabled: state.voiceEnabled, tutorReviews: state.tutorReviews }) }));
